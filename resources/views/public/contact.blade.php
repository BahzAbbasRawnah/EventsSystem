    ...
    <div class="container py-3">
        ...
        <div class="row">
            <div class="col-md-6 d-flex justify-content-center flex-column align-items-center">
                <h2 class="text-dark">{{ __('pages.contact_title') }}</h2>
                <p class="text-dark">{{ __('pages.contact_description') }}</p>
            ...
            <div class="col-md-6 mt-2 m-md-0">
                <h2 class="text-dark">{{ __('pages.form_title') }}</h2>
                <form action="{{ route('contact.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label for="name" class="form-label text-dark">{{ __('pages.form.name') }}</label>
                        <input type="text" class="form-control" id="name" name="name" placeholder="{{ __('pages.form.name_placeholder') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="email" class="form-label text-dark">{{ __('pages.form.email') }}</label>
                        <input type="email" class="form-control" id="email" name="email" placeholder="{{ __('pages.form.email_placeholder') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="message" class="form-label text-dark">{{ __('pages.form.message') }}</label>
